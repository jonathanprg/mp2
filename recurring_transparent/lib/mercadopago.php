...
class MP {
    ...
    public function get_access_token() {
        ...
        //print_r($this->access_data['access_token']);
    ...
    */    
    ...
    public function get_cardtoken($data) {
                
        $url="https://pagamento.mercadopago.com/card_tokens?access_token=". $this->get_access_token() ;
         
        
        $data = json_encode($data);
                        
        $conn = curl_init($url);
        curl_setopt($conn, CURLOPT_USERAGENT, "MercadoPago PHP SDK v" . MP::version);
        curl_setopt($conn, CURLOPT_CAINFO, $GLOBALS["LIB_LOCATION"] . "/cacert.pem");
        //curl_setopt($conn, CURLOPT_SSLVERSION, 3);
        curl_setopt($conn, CURLOPT_POSTFIELDS, $data);
        curl_setopt($conn, CURLOPT_RETURNTRANSFER, true);
        curl_setopt($conn, CURLOPT_CUSTOMREQUEST, "POST");
        curl_setopt($conn, CURLOPT_HTTPHEADER, array("Accept: application/json", "Content-Type: application/json"));
        
        $api_result = curl_exec($conn);
        $api_http_code = curl_getinfo($conn, CURLINFO_HTTP_CODE);
            
        //print_r($api_http_code);
        //print_r($api_result);
     ...
     public function create_charge_payment($preapproval_payment) {
        $access_token = $this->get_access_token();

        $preapproval_payment_result = MPRestClient::post("/authorized_payments?access_token=" . $access_token, $preapproval_payment);
